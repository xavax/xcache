 ...
 * store queue.
 * 
 * There are several objects involved. - A store queue entry encapsulates a key,
 * value, and expire time. To minimize object creation overhead, a fixed number
 * of store queue entries are created and reused. - A concurrent hash map stores
 * data waiting in the queue. It is used as a look-aside cache. - There are
 * separate queues for busy and free store queue entries. - A thread pool
 * executor manages the working threads that write data to the cache. As each
 * entry is dispatched, the associated data is removed from the hash map and the
 * entry returned to the free queue.
 * 
...
public class BasicStoreQueue<K, V> extends AbstractStoreQueue<K, V> {
  ...
  /**
   * Initialize a store queue by creating the thread pool executor, separate
   * queues for free and busy store queue entries, and a hash map of data
  ...
  public void start() {
    ...
    freePoolAllocateTimeMetric = new TimeMetric();
    freePoolReleaseTimeMetric = new TimeMetric();
  ...
  /**
   * If the store queue is full, immediately perform the write (caller runs);
   * otherwise, add an entry to the store queue and insert the key and value
   * into the map.
   * 
  ...
  public void doStore(K key, V value, long expires) {
    long startTime = freePoolAllocateTimeMetric.currentTime();
  ...
  /**
   * Complete a store operation in the store queue, then remove the key and
   * value from the map and return the entry to the free queue.
   * 
  ...
  public void doComplete(StoreQueueEntry<K, V> entry) {
    ...
    entry.clear();
    long startTime = freePoolReleaseTimeMetric.currentTime();
    freeQueue.add(entry);
  ...
  @Override
  protected BasicStoreQueueBuilder<K, V> exemplar() {
  ...
  public TimeMetric freePoolAllocateTimeMetric;
  public TimeMetric freePoolReleaseTimeMetric;
}
